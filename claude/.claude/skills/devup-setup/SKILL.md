...
1. `command -v devup`. If missing, install it from the dotfiles source:
   - `install -m 0755 "$HOME/.dotfiles/bin/.local/bin/devup" "$HOME/.local/bin/devup"` (create `~/.local/bin` if needed; confirm it is on PATH, else tell the user to add it). If that source path does not exist, fetch from `https://raw.githubusercontent.com/mcampbellr/.dotfiles/main/plugins/devup/bin/devup`.
